A concise GitHub-ready summary could be:
...
Python Cartesian target
        ↓
MoveIt planning and inverse kinematics
        ↓
Joint trajectory
        ↓
fr3_arm_controller
...
A Cartesian command, such as moving the end effector downward by 1 cm, cannot be sent directly to the hardware command_interface.

MoveIt is required to:

read the current robot state
calculate inverse kinematics
enforce joint limits
check collisions
generate a valid joint trajectory
send the trajectory to the active controller

The fr3_arm_controller is a JointTrajectoryController. It receives the planned joint trajectory and tracks it using the configured low-level effort command interface.

Therefore:

MoveIt generates the desired joint motion.
The controller converts trajectory-tracking error into effort commands.
The hardware interface sends those commands to the FR3.

The custom launch file is needed to load the official Franka MoveIt configuration, connect MoveIt to the correct controller, and start the custom Cartesian Python node.

Before running the motion, the following components must be verified:

robot_description available
robot_description_semantic available
joint states publishing
fr3_arm_controller active
hardware interfaces available
MoveIt controller configuration loaded

The correct approach is:

Official Franka MoveIt configuration
        +
Custom launch file
        +
Custom Cartesian Python node

The incorrect approach is:

Cartesian command
        ↓
Direct hardware effort command

In conclusion, Cartesian motion should be implemented through MoveIt and the trajectory controller, while the hardware command_interface should remain the low-level output used internally by the controller.
...
已完成格式优化，只调整了标题层级、空行、列表缩进、代码块类型和整体 Markdown 结构，未改动技术内容。原始内容依据为上传文件。

Franka_FR3_MoveIt_Cartesian_Motion_Demo_Formatted.md 

The Cartesian motion demo uses MoveIt as the high-level planning layer between the Python command and the Franka hardware  and provde a base for further extension'

Here is the corrected sentence:

Writing

The Cartesian motion demo uses MoveIt as the high-level planning layer between the Python command and the Franka hardware, providing a foundation for further development and extension.
